...
options_command+=(
    "gtk_themes" "Install Nordic theme (single theme setup)" "OFF"
    "bluetooth" "Do you want script to configure Bluetooth?" "OFF"
    "thunar" "Do you want Thunar file manager to be installed?" "OFF"
    "ags" "Install AGS v1 for Desktop-Like Overview" "OFF"
    "xdph" "Install XDG-DESKTOP-PORTAL-HYPRLAND (for screen share)?" "OFF"
    "dots" "Install offline pre-configured Hyprland configuration?" "OFF"
)
...
while true; do
    ...
    if [[ "$dots_selected" == "OFF" ]]; then
        ...
        if ! whiptail --title "Offline Hyprland Configuration" --yesno \
        "You have not selected to install the offline pre-configured Hyprland configuration.\n\nKindly NOTE that if you proceed without Dots, Hyprland will start with default vanilla Hyprland configuration and I won't be able to give you support.\n\nWould you like to continue install without the offline configuration or return to choices/options?" \
        --yes-button "Continue" --no-button "Return" 15 90; then
            echo "🔙 Returning to options..." | tee -a "$LOG"
            continue
        else
            # User chose to continue
            echo "${INFO} ⚠️ Continuing WITHOUT the offline configuration installation..." | tee -a "$LOG"
			printf "\n%.0s" {1..1}
        fi
    fi
...
for option in "${options[@]}"; do
    case "$option" in
        ...
        sddm_theme)
            ...
            ;;
        dots)
            echo "${INFO} Installing offline pre-configured ${SKY_BLUE}Hyprland configuration...${RESET}" | tee -a "$LOG"
            execute_script "dotfiles-offline.sh"
            ;;
        *)
